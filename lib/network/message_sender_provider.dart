...
class CMessageSender extends _$CMessageSender {
  ...
  void _connectWithSettings(ConnectionSettings settings) {
    ...
    Loggers.network.info('Connecting as client');
    final channel = WebSocketChannel.connect(Uri.parse(settings.adress));
    channel.ready.then((_) {
      Loggers.network.info('Connected successfully');
      channel.stream.listen(
        (data) {
          ...
          try {
            ...
            Loggers.network.warning("Data '$data' could not be parsed.");
            return;
          }
          Loggers.network.fine("Data '$data' recieved and parsed sucessfully.");
          if (parsedData.type == NetworkMessageType.setIdentifier) {
            ...
            Loggers.network.warning("Did not know where to forward data '$data' so it is ignored.");
          }
        },
        onError: _handleErrorFromStream,
        onDone: () {
          Loggers.network.info('Websocket disconnected.');
  ...
  void _handleErrorFromStream(Object error, StackTrace stackTrace) {
    if (error is TimeoutException) {
      Loggers.network.info('Could not connect: timeout');
